...
```
[`count_clashes()` measures the number of conflicts in the timetable, such as students having two exams scheduled at the same time.

A value of 0 means a perfect timetable because it indicates there are no clashes, and all exams are scheduled without any conflicts.]
```

**Q2. What does `generate_neighbor()` do? How is the new timetable different from the current one?**

```
[`generate_neighbor()` creates a new timetable by making a small random change to the current timetable, such as swapping time slots of two exams or reassigning one exam.

The new timetable is slightly different from the current one, allowing the algorithm to explore nearby solutions in the search space. ]
```
...
```
[ This line decides whether to accept the new solution. If the new solution has fewer clashes (delta < 0), it is always accepted. Otherwise, it may still be accepted based on a probability.

Simulated Annealing accepts worse solutions to avoid getting stuck in local minima. This probabilistic acceptance helps explore more possible solutions, especially at higher temperatures.]
```
